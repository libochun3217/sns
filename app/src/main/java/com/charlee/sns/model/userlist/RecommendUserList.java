...
/**
 */
public class RecommendUserList extends PageableList<SnsUser, UserInfo> {
    private final SnsUser snsUser;

    public RecommendUserList(@NonNull final IMotuSns motuSns, @NonNull final IModelRepository repository,
    ...
    protected Task<PagedList<UserInfo>> getFirstPage() {
        return motuSns.getUserFollowees(snsUser.getId(), IMotuSns.FIRST_PAGE_ID, IMotuSns.DEFAULT_PAGE_SIZE)
    ...
    protected Task<PagedList<UserInfo>> getNextPage(@NonNull PagedList<UserInfo> before) {
        ...
        return motuSns.getUserFollowees(snsUser.getId(), lastId, IMotuSns.DEFAULT_PAGE_SIZE)
                .onSuccess(continuation);
    }
